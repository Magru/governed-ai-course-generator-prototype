...
#: The notice learners are sent when the course goes live, and what each
#: approver is shown before consenting to it: this text, and how many it reaches.
NOTICE = "Hand tool safety is available: complete it before your next workshop shift."


def signatures(notice: str = NOTICE, recipients: int = 40) -> list[dict]:
    return [{"actor": "admin-1", "role": "training-administrator",
             "what_was_shown": {"notice": notice, "recipients": recipients}},
...
def to_published(m: Machine) -> None:
    ...
    m.fire("LearnersNotified", {"actor": "admin-1", "recipients": 40, "notice": NOTICE,
                                "notice_screening": {
