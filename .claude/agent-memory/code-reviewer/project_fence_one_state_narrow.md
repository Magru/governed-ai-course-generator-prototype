...
Pass 7 (spec-v2.13): turning a terminal failure into "wait" left no driver — check who re-sends the awaited event (grep callers, not just table legality). New class: the temporal engine's `trace.before(i)` is the previous *global* step, so any step on another revision (notice consent, notice) poisons I1 for every later draft. The walk harness should always include events addressed to non-current revisions (`revision=` payload) and run LTL at the end.
